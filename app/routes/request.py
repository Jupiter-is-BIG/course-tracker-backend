...
from .scrape import createDmChannel, sendMessage
...
async def push_request(
    ...
    num_of_existsing_requests = (
        ...
        .filter(
            and_(
                user.UserRequests.user_id == current_user.user_id,
                user.UserRequests.is_active == True,
            )
        )
        .count()
    )
    ...
    if campus != "UBCV" and campus != "UBCO":
        raise HTTPException(
            status_code=status.HTTP_406_NOT_ACCEPTABLE,
            detail="Wrong campus format",
        )
    ...
    if dive:
        previous_record = (
            ...
            .filter(
                and_(
                    user.UserRequests.user_id == current_user.user_id,
                    user.UserRequests.request_id == dive.request_id,
                )
            )
            .first()
        )
        ...
        if previous_record:
            ...
            new_record = user.UserRequests(
                user_id=current_user.user_id, request_id=dive.request_id, is_active=True
            )
            db.add(new_record)
        ...
        return {
            "message": f"Resquest for {subject} {code} {section} on {campus} campus has been registered successfully"
        }
    ...
    new_record = user.UserRequests(
        user_id=current_user.user_id, request_id=new_request.request_id, is_active=True
    )
    db.add(new_record)
    db.commit()
